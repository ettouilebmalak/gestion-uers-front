...
function AdminCentral({ keycloak }) {
    ...
    {/*selected pour savoir la section selectionne admins ou users*/}
    const [selected, setSelected] = useState("");
    {/*stockage de texte recherche dans la barre*/}
    const [searchRole, setSearchRole] = useState("")
    {/* pour savoir est ce qu'on est dans liste ou creation user*/}
    ...
    {/*changement du section de menu puis revenir a la lste apres creation */ }
    const handleMenuChange = (e) => {
        setSelected(e.target.value);
        setView("list");
    };
    {/* creation d'un nouveau user via form*/}
    const handleCreateUser = (e) => {
        e.preventDefault();

        const form = e.target;
       {/* new user a partir des infos stocke dans le form*/}
        const newUser = {
            id: users.length + 1,
            nom: form.nom.value,
    ...
    {/* suppression user pa id*/}
    const handleDelete = (id) => {
        const newList = users.filter(user => user.id !== id);
        setUsers(newList);
    };
    {/*filtrer users par role*/}
    ...
    return (
        <>
           {/*header commun entre tous les compoants*/}
            ...
            <div style={{ display: "flex", paddingTop: "70px" }}>
                <div style={{ display: "flex" }}>
                    ...
                    <div style={{ padding: "40px", flex: 1 }}>
                        {/*si aucune section n choisis affice msg de bienvenue*/}
                        {!selected && (
                        ...
                        {/*choisir administrations afficher list des admins*/}
                        ...
                        {selected === "users" && view === "list" && (
                            <>
                                ...
                                <table border="1" width="300%" style={{ marginTop: "10px" }}>
                                    ...
                                    <tbody>
                                        {/*mapper sur users filtre par role*/}
                                        {filterRole.map((user) => (
                                            <tr key={user.id}>
                                                <td>{user.nom}</td>
                                                <td>{user.prenom}</td>
                        ...
                        {/*choisir users et afficher le formulaire*/}
